...
class TrSlider {
    ...
    emblaDotsSetUp() {
        ...
        const selectDotBtn = (dotsArray, embla) => {
            ...
            dotsArray[previous].classList.remove(
                emblaDefaultOptions.selectedClass,
            );
            dotsArray[selected].classList.add(
                emblaDefaultOptions.selectedClass,
            );
        };
    ...
    reconfigureOptions(checkBreakpoints = false) {
        ...
        if (checkBreakpoints) {
            const breakPointsHighToLow = [
                ...Object.keys(this.breakpoints),
            ].sort((a, b) => +b - +a);

            breakPointsHighToLow.forEach((maxWidthVal) => {
                if (
                    window.matchMedia(`(max-width: ${maxWidthVal}px)`).matches
                ) {
                    this.finalOptions = {
                        ...this.finalOptions,
        ...
        if (this.finalOptions.autoplay) {
            ...
            console.log('DETECTED SLIDER', this.emblaNode);
            this.autoplay().play();
        } else {
